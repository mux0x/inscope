...
var scopeFile = flag.String("scope", "", "full path to a .scope file (optional)")

func main() {
	flag.Parse()

	sf, err := openScopefile(*scopeFile)
...
func openScopefile(arg string) (io.ReadCloser, error) {
	if arg != "" {
		f, err := os.Open(arg)
		if err != nil {
	...
	return nil, errors.New("unable to find .scope file (use --scope to specify one)")
}
